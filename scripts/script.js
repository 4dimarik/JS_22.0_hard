...
const { width: blockWidth } = getComputedStyle(document.querySelector(blockSelector));
const blockNum = squareBody.querySelectorAll(blockSelector).length;
const blockNumInRow = Math.trunc(+squareWidth.replace('px', '') / +blockWidth.replace('px', ''));
  ...
  if (arrow) {
    ...
    if (arrow.matches('.arrow.top')) {
      ...
      if (index >= blockNumInRow) {
        ...
        nextBlock.insertAdjacentElement('beforebegin', currentBlockAtNewPosition);
      }
